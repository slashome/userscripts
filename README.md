...
| **Lucca trombinoscope quiz** | Adds a "Quiz" button next to "Organigramme" on the Lucca directory: photos show up one by one and you guess each person's first name (1 point) and last name (3 points for both). The final screen recaps every name and highlights the ones you missed. | [Install](https://raw.githubusercontent.com/slashome/userscripts/main/scripts/lucca-trombi-quiz.user.js) |
